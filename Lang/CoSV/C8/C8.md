...
<i>Note 2: "Curve Integrals" are more commonly known as "Line Integrals," but Prof. Lang’s term is more accurate.</i>

<b>Section 1: Definition and Evaluation</b>

<b>1)</b> Calculate $\int_{\gamma} F(x,y) \cdot d\gamma$ where $F(x,y) = (x^2-2xy, y^2-2xy)$ and $\gamma = (x, x^2)$ from $x=-2$ to $1$.
...
& = & \frac{kc^2}{b+2} + \frac{bk^3c^3}{3b+6} = \frac{3kc^2 + k^3c^3b}{3(b+2)} = \frac{k^3c^3(\frac{3}{k^2c}+b)}{3(b+2)} \\
\end{eqnarray}$

$\therefore$ if $\frac{3}{k^2c} = 2 \implies k = \sqrt{\frac3{2c}} \implies \boxed{a = \sqrt{\frac{3c}2}}$ then the integral will be independent of $b.~~~\blacksquare$
<br><br>
...
Note that the integral is independent of $r$, thus the radius of the circle doesn’t matter (as long as it’s origin-centered); and otherwise, the dot product remains the same&mdash;1&mdash;thus the integrand is always $dt$ and the integral is thus always equal to the (signed) angular distance traveled.&nbsp; This makes the answers to parts b) - d) trivial:
<br><br>
...
$\displaystyle \int_{-1}^2 ((2y^2)y, 2y^2) \cdot (4ydy, dy) = \int_{-1}^2 (8y^4 + 2y^2)dy = \left[\frac85 y^5 + \frac23 y^3\right]_{-1}^2 = \frac{256}5 + \frac{16}3 - \left(-\frac85 - \frac23\right) = \boxed{\frac{264}5 + 6 = \frac{294}5}$
<br><br>

<b>Section 3: When the vector field has a potential function</b>
...
Next, we ask: "is the connected, open interior of the path of integration a rectangle?"&nbsp; Since the answer is yes, we conclude that $\boxed{F~does}$ admit a potential function on the specified domain.
...
<b>Sln</b>: No, because, by the result of part b), there exists a non-rectangular, closed curve around which the curve integral of $F$ is non-zero.
...
<b>a)</b> $\int_{(2,1)}^{(-3,4)} F \cdot dC, O \notin C$.
...
<b>Sln</b>: $F(x,y) = \displaystyle \frac1r(xe^r, ye^r) = f(X) = \frac{g’(r)}{r}X, g’(r) = e^{r}, r = ||X|| \underset{\text{C4 S4}}{\implies} \exists \varphi(X) = g(||X||) = \int e^rdr = e^r : F = \nabla\varphi$
 ...
 $\therefore \int_{(2,1)}^{(-3,4)} F \cdot dC = e^{||(-3,4)||} - e^{||(2,1)||} = \boxed{e^5 - e^{\sqrt5}}$.

For b)-d), $C(x,y) := x^2 + y^2 = 2^2$

<b>b)</b> $\int_{(2,0)}^{(0,2)} F \cdot dC$.

<b>c)</b> $\int_{(2,0)}^{(\sqrt2,\sqrt2)} F \cdot dC$.

<b>d)</b> $\oint F \cdot dC$.

<b>Sln</b>: In all three of these integrals, the beginning point and the end point are the same distance from the origin, and since the potential function exists and is a function only of distance from the origin, all three integrals equal $\boxed{0}$.
<br><br>

<b>16)</b> Integrate $F(x,y,z) = (x, y^2, 4z^3)$ from $(0,0,0)$ to $(1,1,2)$ along the path pictured in the Exercise statement in the text.

<b>Sln</b>: Though it’s not difficult to do so (see our [Selected Solutions for Chapter 1](http://www.selectedsolutions.net/Lang/CalcOfSevVar/C1/C1.html) of this text for examples of how to do this), I’m not bothering to give a parameterization of the path because the whole point of the Exercise is that it’s unnecessary: $F$ admits a potential function, $\varphi(x,y,z)$, which is readily seen to be $\displaystyle \varphi(x,y,z) = \frac{x^2}2 + \frac{y^3}{3} + z^4 \therefore$ the required integral is simply:

$$\varphi(1,1,2) - \varphi(0,0,0) = \frac12 + \frac13 + 2^4 - 0 = \boxed{16 + \frac56 = \frac{101}{6}.}$$

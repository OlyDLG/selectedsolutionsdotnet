...
__Pf__: For the case $\lvert z\rvert  = 1$, the limit exists for some $z$, e.g., $z = 1: f(1) = \Lim_{n \rarrow \infty} \Frac{1^n -1}{1^n + 1} = 0$, but not for others, e.g., $z = -1 :$ $f(-1) = \Frac{(-1)^n -1}{(-1)^n + 1}$ which jumps back and forth between $0$ and $\infty$, so convergence for $\lvert z\rvert  = 1$ is at best conditional. (In fact, for &quot;almost all&quot; $z : \lvert z\rvert  = 1$, the ___orbit___ of $z$ under the map $f(n) = z^n$ is [___dense___ in the unit circle](https://en.wikipedia.org/wiki/Irrational_rotation) $\implies$ the limit does not exist for all such points; but the proofs of both claims are beyond our present scope.)

Now if $\lvert z\rvert  = r \in [0,1)$, $\Lim_{n \rarrow \infty} z^n = \Lim_{n \rarrow \infty}r^ne^{i n\t} = 0$ (since $\forall \e \gt 0, \exists N \in \mathbb{N} : n \ge N \implies \lvert r^ne^{i n\t}\rvert  = r^n\cdot 1 = r^n \lt \e$ by a fundamental property of the reals) so $\Lim_{n \rarrow \infty}\Frac{z^n -1}{z^n + 1} = \Frac{0-1}{0+1} = -1.$

Finally, if $\lvert z\rvert  = r \in (1,\infty)$, consider that $\Lim_{n \rarrow \infty} \Frac{z^n -1}{z^n + 1} = \Lim_{n \rarrow \infty} \Frac{(1/z)^{-n} - 1}{(1/z)^{-n} + 1} = \Lim_{n \rarrow \infty} \Frac{w^{-n} - 1}{w^{-n} + 1}, 0 \lt \lvert w\rvert  \lt 1$; multiply the numerator and denominator by $w^n \ne 0$ to see that this equals $\Lim_{n \rarrow \infty} \Frac{1 - w^n}{1 + w^n} = -\Lim_{n \rarrow \infty} \Frac{w^n - 1}{w^n + 1} = -(-1) = 1$ (since $\lvert w\rvert  \lt 1).~~~\blksq$<br>

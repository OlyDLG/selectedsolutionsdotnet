...
and the desired result follows from an elementary fact about the reals.
...
<br>
Pf: $\lvert z + w\rvert = \lvert z - (-w)\rvert \ge \lvert z\rvert - \lvert -w\rvert = \lvert z\rvert - \lvert w\rvert$
...
Pf: $\alpha^{1/2} = \pm\lvert\alpha\rvert^{1/2}\exp(i\arg(\alpha)/2) = \lvert\alpha\rvert^{1/2}\exp[i(\arg(\alpha)/2 + k\pi)], k\in\mathbb{Z}$, but only any $k$-consecutive pair of the second factor, i.e., $\exp[i(\arg(\alpha)/2+k\pi)], \exp[i(\arg(\alpha)/2 + (k+1)\pi)]$ are distinct. (Note that the only reason to exclude $0$ is because it is its own square-root and it is known to be unique.)

<br>
1.2.4) Find ${(x(a, b), y(a, b)): (x + iy)^2 = a + ib}$

$a + ib = (a^2 + b^2)^{1/2}\exp[i(\arctan(b/a)+2k\pi)] = (x + iy)^2 \implies$
$x + iy = (a^2 + b^2)^{1/4}\exp[i(\arctan(b/a)/2+k\pi)] =$
$(a^2+b^2)^{1/4}[\cos(\arctan(b/a)/2+k\pi) + i\sin(\arctan(b/a)/2+k\pi)], k \in \mathbb{Z}, \therefore$
$x(a, b) = (a^2 + b^2)^{1/4}\cos(\arctan(b/a)/2+kπ), \\y(a, b) = (a^2 + b^2)^{1/4}\sin(\arctan(b/a)/2+kπ), k\in\mathbb{Z}$

<br>
1.2.5) Plot $\{ z: z^n = 1\} \text{ for } n = 2, 3, 4, 5$

$n=2$: <br><img src="./Lang/CA/C1/SqrRts.png">

$n=3$: <br><img src=http://www.selectedsolutions.net/Lang/ComplexAnalysis/CubeRts.png>


$n=4$: <br><img src=http://www.selectedsolutions.net/Lang/ComplexAnalysis/FourthRts.png>


$n=5$: <br><img src=http://www.selectedsolutions.net/Lang/ComplexAnalysis/FifthRts.png>
...
**Uniqueness**: However, but for $z = 0$ (which is unique), if $z_j = \lvert \alpha\rvert^{1/n}\exp[i(\arg(\alpha)+2j\pi)/n]$ such that $j \equiv k\pmod n, k \in\{0, 1, ..., n-1\}$, i.e., $\exists m \in \mathbb{Z} : j = m\cdot n + k$, then 
...
<br>there are precisely $n$ distinct such $z_j$.
...
c) $e^z = e^w \implies e^{w-z} = 1 \implies \exists k \in \mathbb{Z} : e^{w-z} = e^{2ik\pi} \implies w-z = 2ik\pi~~\checkmark$.
...
Pf: Since the identity $\sum_{n=0}^N z^n = (z^{N+1}-1)/(z-1)$ is true $\forall z \in \mathbb{C}\setminus\{1\}$, it is true $\forall z=e^{i\theta}, 0\lt\theta\lt2\pi.~$ For such $z$, it is "straightforward" to show that the real part of the left-hand-side (LHS) of the given identity is precisely the LHS of the identity to be shown; the "challenging" part is to show the right-hand-side (RHS). Substituting $e^{i\theta}$ in for $z$ in the RHS of the given identity, we have: $$\frac{e^{i(N+1)\theta}-1}{e^{i\theta}-1} = \frac{e^{i(N+1)\theta/2}}{e^{i\theta/2}}\frac{e^{i(N+1)\theta/2}-e^{-i(N+1)\theta/2}}{e^{i\theta/2}-e^{-i\theta/2}}=e^{iN\theta/2}\frac{\sin[(N+1)\theta/2]}{\sin(\theta/2)}.$$  Since $\frac{\sin[(N+1)\theta/2]}{\sin(\theta/2)}$ is real, the real part of the RHS is this times the real part of $e^{iN\theta/2}$, i.e., $\cos(\frac{N\theta}{2})$, and thus, using various trigonometric identities, we have Re(RHS) $$=\cos\left(\frac{N\theta}{2}\right)\frac{\sin\frac{N\theta}{2}\cos\frac{\theta}{2}+\cos\frac{N\theta}{2}\sin\frac{\theta}{2}}{\sin\frac{\theta}{2}}=\frac{\frac{1}{2}\sin N\theta\cos\frac{\theta}{2}+\cos^2\frac{N\theta}{2}\sin\frac{\theta}{2}}{\sin\frac{\theta}{2}}$$
...
$\lim_{n\rightarrow\infty}z_n=w \iff \lim_{\substack{z\rightarrow 0 \\ z \in S}} f(z) = w.$
...
<br>
1.4.3) Show that $\lvert z \rvert \lt 1 \implies \sum_{n=0}^{\infty} z^n = (1-z)^{-1}$.

Pf: The Reader should have already shown that $\sum_{n=0}^N z^n = (z^{N+1} - 1)/(z-1)$, if not previously, at least in Exercise 1.2.11. (Hint: let $S = \sum_{n=0}^{N-1} z^n$, multiply by $z$, subtract $S$ from $zS$,and solve for $S$.) So $\sum_{n=0}^{\infty} z^n = \lim_{N \rightarrow \infty} \sum_{n=0}^N z^n = \lim_{N \rightarrow \infty} (z^{N+1} - 1)/(z-1) = [\lim_{N \rightarrow \infty} (z^{N+1} - 1)]/[\lim_{N \rightarrow \infty} (z-1)] = [(\lim_{N \rightarrow \infty} z^{N+1}) - 1]/(z-1).~$ Now, by the result of 1.4.1, if $\lvert z \rvert \lt 1,~\lim_{N \rightarrow \infty} z^{N+1} = 0 \therefore [(\lim_{N \rightarrow \infty} z^{N+1}) - 1]/(z-1) = -1/(z-1) = (1-z)^{-1}$. $\checkmark$ 

<br>
1.4.4) Given: $f(z) = \lim_{n \rightarrow \infty} 1/(1 + n^2 z)$, show: $f(0) = 1,~f(z \ne 0) = 0$.

Pf: $f(0) = \lim_{n \rightarrow \infty} 1/(1+n^2(0)) = \lim_{n \rightarrow \infty} 1/(1+0)~\therefore \lim_{n \rightarrow \infty} 1/(1+n^2(0)) = 1/1 = 1$;<br>
$f(z \ne 0) = \lim_{n \rightarrow \infty} 1/(1+n^2z) = \lim_{n \rightarrow \infty} \frac{1/n^2}{(1/n^2+z)} = \frac{\lim_{n \rightarrow \infty}1/n^2}{\lim_{n \rightarrow \infty}(1/n^2 + z)} = \frac{0}{0 + z} = 0$. 
...
<br>
1.4.7) Show that $\sum \frac{z^{n-1}}{(1-z^n)(1-z^{n+1})}$ converges (uniformly) to $\Bigg\{\!
  \begin{array}{c}
...
Sln: First note that the summand is undefined (or infinite) $\forall z$ if $n=0$, so we assume that the sum starts from $n=1.~$ Let $A_n(z) = \frac{z^{n-1}}{(1-z^n)(1-z^{n+1})}$ and consider the partial sum $\sum_{n=1}^{N} A_n(z).~$ Computing&mdash;and simplifying&mdash;this for a few small values of $N$ leads to the following conjecture:
$$\sum_{n=1}^N A_n(z) = \frac{\sum_{m=0}^{N-1} z^m}{(1-z)(1-z^{N+1})},$$which we will now prove by induction.

First note that it is true for $N=1$: $\sum_{n=1}^1 A_n(z) = A_1(z) = \frac{1}{(1-z)(1-z^2)} = \frac{\sum_{m=0}^{1-1}z^m}{(1-z)(1-z^{1+1})}.~$ Now we will show that "true for $N$ implies true for $N+1$":

$\sum_{n=1}^{N+1} A_n(z) = \sum_{n=1}^{N} A_n(z) + \frac{z^N}{(1-z^{N+1})(1-z^{N+2})} = \frac{z^N}{(1-z^{N+1})(1-z^{N+2})} + \frac{\sum_{m=0}^{N-1}z^m}{(1-z)(1-z^{N+1})}$ (this is where we've used "assumed true for $N$")
<br>$~~~= \frac{z^N}{(1-z^{N+1})(1-z^{N+2})} + \frac{1-z^N}{(1-z)(1-z)(1-z^{N+1})} = \frac{z^N(1-z)^2 + (1-z^{N+2})(1-z^N)}{(1-z)^2(1-z^{N+1})(1-z^{N+2})} = \frac{1+z^N-z^N-2z^{N+1}+z^{N+2}-z^{N+2}+z^{2N+2}}{(1-z)^2(1-z^{N+1})(1-z^{N+2})} = \frac{(1-z^{N+1})^2}{(1-z)^2(1-z^{N+1})(1-z^{N+2})} = \frac{1-z^{N+1}}{(1-z)^2(1-z^{N+2})}$
$$=\frac{\sum_{m=0}^N z^m}{(1-z)(1-z^{N+2})}$$<br>which is our formula evaluated for $N+1.~$ Consequently, $\lim_{N\rightarrow \infty} \sum_{n=1}^N A_n(z) = \lim_{N\rightarrow \infty}\frac{\sum_{m=0}^{N-1} z^m}{(1-z)(1-z^{N+1})} = \lim_{N\rightarrow \infty} \frac{1-z^N}{(1-z)^2(1-z^{N+1})} =$ $$\frac{1}{(1-z)^2}$$ when $\left|z\right| \lt 1.~$ The other three parts of the problem are "left to the Reader."
...
$hu_x + ku_y + ihv_x + ikv_y + o(h,k) = hu_x - kv_x + ihv_x + iku_x + o(h,k) = (h+ik)(u_x +iv_x) + o(h,k).~$ Again utilizing the given that $u, v \in C^1, u_x, v_x$ are well-defined, i.e., $u_x + iv_x$ is a well-defined complex number, $a$; dividing by $w$ and subtracting $a$ yields:<br> $\sigma(w) = \frac{f(z+w) -f(z)}{w} - a = o(w).~~~~~$QED

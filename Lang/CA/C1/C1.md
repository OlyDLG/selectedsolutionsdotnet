...
$\ds \lim_{n\rightarrow\infty}z_n=w \iff \lim_{\substack{z\rightarrow 0 \\ z \in S}} f(z) = w.$ We shall provide a proof.<br>
...
$\implies$: To show that $\ds \lim_{\substack{z\rightarrow 0 \\ z \in S}} f(z) = w$, we must show:<br>
a) $f$ is well&#8209;defined on $S$: given.<br>
b) $0$ is adherent to $S$: we need to show that every disc $D(0, r\gt 0)$ contains some point of $S$. $D(0, r \ge 1)$ contains $1 \in S$ so we confine our attention to $0 \lt r \lt 1$. For any such $r$, take $N \gt \Frac1r$; then $\Frac1N \in S \lt \Frac1r \implies \Frac1N \in D(0,r \gt 0).$<br>
c) $(\ds \lim_{n\rightarrow\infty}z_n=w \equiv \forall \e \gt 0~\exists N \in \mathbb{N} : n \in \mathbb{N} \ge N \rarrow \left|z_n - w\right| \lt \e) \implies (\lim_{\substack{z\rightarrow 0 \\ z \in S}} f(z) = w \equiv \forall \e \gt 0~\exists \d \gt 0 : z \in S, \left|z\right| \lt \d \rarrow \left|f(z) - w\right| \lt \e)$<br>
...
$(\forall \e \gt 0~\exists N \in \mathbb{N} : n \in \mathbb{N} \ge N \rarrow \left|z_n - w\right| \lt \e) \implies 
\forall \e \gt 0~\exists N : n \ge N \rarrow \left|f(1/n) - w\right| \lt \e~[z_n = f(1/n) \text{ given}] \implies$ 
$\forall \e \gt 0~n \gt N \rarrow \left|f(1/n) - w\right| \lt \e~[\text{true for } n \ge N \rarrow \text{ true for } n \gt N] \implies 
\forall \e \gt 0,~0 \lt \Frac1n \lt \Frac1N \rarrow \left|f(1/n) - w\right| \lt \e$ $[0 \lt \Frac1n \lt \Frac1N \rarrow n \gt N \gt 0].~$ 
Now choose $\d = \Frac1N \gt 0~[\text{axiom of choice, } \Frac1N \gt 0]$ 
and let $z = \Frac1n \implies z \in S, \left|z\right| \lt \d~[$definition of $S$, $\Frac1n \lt \Frac1N] \implies 
\forall \e \gt 0~\exists \d \gt 0 : z \in S, \left|z\right| \lt \d \rarrow \left|f(z) - w\right| \lt \e,$ 
which is what we were required to show.

$\Longleftarrow$: $(\forall \e \gt 0~\exists \d \gt 0 : z \in S, \left|z\right| \lt \d \rarrow \left|f(z) - w\right| \lt \e) \implies$ $\forall \e \gt 0~\exists \d \gt 0 : \Frac1n \lt \d \rarrow \left|f(1/n) - w\right| \lt \e$ $[z \in S \rarrow \exists n \in \mathbb{N} : z = \Frac1n = \left|\Frac1n\right| = \left|z\right| \lt \d; z = \Frac1n \rarrow f(z) = f(1/n)].~$ Now choose $N = \left\lceil \Frac1{\d}\right\rceil \ge 1$ where $\lceil x \rceil$ is the least natural number ___strictly exceeding___ $\Frac1{\d}~[$axiom of choice; $\d \gt 0 \rarrow \Frac1{\d}$ well&#8209;defined and $\left\lceil \Frac1{\d}\right\rceil \ge 1];$ $\left\lceil \Frac1{\d}\right\rceil \gt \Frac1{\d}~[$given our definition of $\lceil x \rceil] \implies 0 \lt \Frac1{\lceil1/\d\rceil} \lt \d \implies n \ge N \rarrow 0 \lt \Frac1n \le \Frac1N = \Frac1{\lceil1/\d\rceil} \lt \d$ and we established above that $\forall \e \gt 0~\exists \d \gt 0 : \Frac1n \lt \d \rarrow \left|f(1/n) - w\right| \lt \e$, so now we have that $\forall \e \gt 0~\exists N = \left\lceil\Frac1{\d}\right\rceil : n \ge N \rarrow \left|f(1/n) - w\right| \lt \e;$ setting $z_n = \Frac1n$ yields $\forall \e \gt 0~\exists N : n \ge N \rarrow \left|f(z_n) - w\right| \lt \e$ as required.$~~\blksq$
<br><br>
...
Show that $\left|z\right| \ne 1 \implies \Lim_{n \rarrow \infty} \Frac{z^n -1}{z^n + 1}$ exists.

__Pf__: For the case $\left|z\right| = 1$, the limit exists for some $z$, e.g., $z = 1: f(1) = \Lim_{n \rarrow \infty} \Frac{1^n -1}{1^n + 1} = 0$, but not for others, e.g., $z = -1: \Frac{(-1)^n -1}{(-1)^n + 1}$ which jumps back and forth between $0$ and $\infty$, so convergence for $\left|z\right| = 1$ is at best conditional. (In fact, for &quot;almost all&quot; $z : \left|z\right| = 1$, the ___orbit___ of $z$ under the map $f(n) = z^n$ is [___dense___ in the unit circle](https://en.wikipedia.org/wiki/Irrational_rotation) $\implies$ the limit does not exist for all such points; but the proofs of both claims are beyond our present scope.)

Now if $\left|z\right| = r \in [0,1)$, $\Lim_{n \rarrow \infty} z^n = \Lim_{n \rarrow \infty}r^ne^{i n\t} = 0$ (since $\forall \e \gt 0, \exists N \in \mathbb{N} : n \ge N \implies \left|r^ne^{i n\t}\right| = r^n \lt \e$ by a fundamental property of the reals) so $\Lim_{n \rarrow \infty}\Frac{z^n -1}{z^n + 1} = \Frac{0-1}{0+1} = -1$.

Finally, if $\left|z\right| = r \in (1,\infty)$, consider that $\Lim_{n \rarrow \infty} \Frac{z^n -1}{z^n + 1} = \Lim_{n \rarrow \infty} \Frac{(1/z)^{-n} - 1}{(1/z)^{-n} + 1} = \Lim_{n \rarrow \infty} \Frac{w^{-n} - 1}{w^{-n} + 1}, 0 \lt \left|w\right| \lt 1$; multiply the numerator and denominator by $w^n \ne 0$ to see that this equals $\Lim_{n \rarrow \infty} \Frac{1 - w^n}{1 + w^n} = -\Lim_{n \rarrow \infty} \Frac{w^n - 1}{w^n + 1} = -(-1) = 1$ (since $\left|w\right| \lt 1).~~~\blacksquare$<br>

(Thus $f(z) = \left\{
\begin{array}{ll}
-1, & \left|z\right| \lt 1 \\ 
\text{undef}, & \left|z\right| = 1 \\
1, & \left|z\right| \gt 1 
\end{array}
\right.$, so if we __define__ $f(z, \left|z\right|=1) \equiv 0$ so that $f(z)$ becomes $f(z) = \left\{
\begin{array}{rr}
-1, & \left|z\right| \lt 1 \\ 
0, & \left|z\right| = 1 \\
1, & \left|z\right| \gt 1 
\end{array}
\right.$<br> 
then, except for $z=0, f(z) = \text{sign}(\log\left|z\right|)$, i.e., we have an &quot;algebraic formula&quot; for this important piecewise&#8209;continuous function.)
<br><br>
...
$$=\frac{\sum_{m=0}^N z^m}{(1-z)(1-z^{N+2})}$$<br>which is our formula evaluated for $N+1$. Consequently, $\displaystyle \lim_{N\rightarrow \infty} \sum_{n=1}^N A_n(z) = \lim_{N\rightarrow \infty}\frac{\sum_{m=0}^{N-1} z^m}{(1-z)(1-z^{N+1})} = \lim_{N\rightarrow \infty} \frac{1-z^N}{(1-z)^2(1-z^{N+1})} =$ $$\frac{1}{(1-z)^2}$$ when $\left|z\right| \lt 1$. 

For $|z| \gt 1$, begin by multiplying the numerator and denominator of $(*)$ by $z^{-N + 1} \ne 0$:<br>
$\ds \sum_{n=1}^N A_n(z) = \frac{\left(\sum_{m=0}^{N-1} z^m\right)z^{-N+1}}{(1-z)(1-z^{N+1})z^{-N+1}} = \frac{\sum_{m=0}^{N-1} z^{m-N+1}}{(1-z)(z^{-N+1}-z^2)} = \frac{z^{-N+1} + z^{-N+2} + \dots + z^{-1} + 1}{z^2(1-z)(z^{-N-1}-1)} = \frac{\sum_{m=0}^{N-1}(1/z)^m}{z^2(1-z)[(1/z)^{N+1}-1]}.$ Now $\ds \left|z\right| \gt 1 \implies \left|\frac1z\right| \lt 1 \implies \lim_{N\rarrow\infty}\sum_{m=0}^{N-1}z^{-m} = \frac{1-z^{-N}}{1-z^{-1}} \implies \lim_{N\rarrow\infty}\frac{\sum_{m=0}^{N-1}z^{-m}}{z^2(1-z)(z^{-(N+1)}-1)} = \lim_{N\rarrow\infty}\frac{1-z^{-N}}{(1-z^{-1})z^2(1-z)(z^{-(N+1)}-1)} =$ $\Frac{1}{z^2(2 - z - z^{-1})(-1)} = \Frac1{z(z^2 - 2z + 1)} = \Frac{1}{z(1-z)^2}.~~~\blksq$ (Proof that the convergence is uniform is left to the reader.)
<br><br>

...
<div class="tqimg"><img src="./Graphs/LangC1S2E5.png"></div>
...
__Sln__: Substituting, we have: $f(z(s,t;B))=\exp[2\pi i(s + i(B+t))] = e^{-2\pi(B+t)}e^{2\pi is}$. For fixed $s_0 \in \left[-\Frac12,\Frac12\right]$, i.e., the vertical ray $z(t) = s_0 + i(B + t), t \in [0,\infty), B \gt 0$, this is the &quot;half-open&quot; radial segment, $w(t) = e^{-2\pi(B+t)}e^{2\pi i s_0}$, from $e^{-2\pi B}e^{2\pi i s_0}$ (inclusive) to $w=0$ (the origin, exclusive); note that the larger $B$ is, the smaller is the radius. 

For fixed $t_0 \in [0, \infty)$, i.e., the horizontal closed segment $z(s) = s + i(B + t_0), s \in \left[-\Frac12,\Frac12\right]$, this is the circle, $w(s) = e^{-2\pi(B+t_0)}e^{2\pi is}$, of radius $e^{-2\pi(B+t_0)}$, with the (real-axis) point $w = -e^{-2\pi(B+t_0)}$ being &quot;hit&quot; twice. Thus, $S$ is mapped onto (but not precisely 1-1) the origin-centered-but-excluding disc of radius $e^{-2\pi B}$. (As we will see shortly, the fact that orthogonal curves in $S$&mdash;e.g., horizontal and vertical (subsets of) lines&mdash;are mapped to orthogonal curves in $f(S)$&mdash;e.g., concentric circles and their radii&mdash;is not a coincidence: it is a signature consequence of $f(z)=e^{2\pi iz}$ being __holomorphic__.)

...
$~~~~$Investigating $\beta=0$ first, we almost omitted $x_0=0$ from that parameter’s provided options, because when $\beta=0$, $x=0$ is a fixed point for any values of the other parameters: $x_{i+1}(0) = \left[\alpha (0) b^n + 0\right] (\text{mod }b^n) / b^n = 0(\text{mod }b^n) /b^n$ (for both Models) $= 0/b^n = 0$. However, this result only holds for $\beta=0$: set $beta \ne 0$ and even $\alpha=1$&mdash;which one might guess wouldn’t be very interesting&mdash;produces "saw-tooth" waves, including saw-tooth wave products, some of which (e.g., $\beta=101, \alpha=1, x_0=0, b=3, n=5$) might be confused for pseudo-random number sequences if the user is not careful. There are plenty of other interesting sequences starting with $x_0=0$ as long as $\beta\ne 0$.

$~~~~$We leave it to the reader to find parameter combinations which appear to produce "good" pseudo-random sequences; we <i>will</i> note, however, that the more the phase plane graph of such a sequence appears to "chaotically fill" the domain $[0,1) \times [0,1)$, the more the sequence will appear to be random (the reader should spend time understanding why this is so). More interesting to us is the variety of periodic sequences these Models  can produce; here’s just a sampling:

With $b=2, n=8$: $\alpha=15, \beta=1$ (any initial value, period 32, both Models); 

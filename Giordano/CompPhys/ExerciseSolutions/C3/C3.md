...
$~~~~$(For convenience, henceforth, except in the graph labels, we use $x, v$ in place of $\theta, \omega$) For the physical pendulum model (PPM):
...
$~~~~$The page loads with graphs, below, of $x(t), v(t)$ (the time domain solution) on one set of axes and $v(x)$ (the phase plane solution) on another set of axes; as advised in the text, the default values used are those used to generate text Figures 3.6 and 3.9. A check box above the phase plane graph switches to graphs of the "targeted" strange attractor and a "close-up" of the attractor for values of $x \gt 2$ (the $v$ range is automatically determined by the graphing package to include all points satisfying the $x \gt 2$ condition). In both views, the user can change $F_D$ in increments of $0.01$ between $1.16$ and $1.24$ (nine possible values) and $\Omega_D$ in increments of $1/48$ between $28$ and $36$ (nine possible values) for a total of $9\times9=81$ different $(F_D,\Omega_D)$ pairings. (We originally implemented similar user-selectability for $q$ and $\ell$, but found that small variations in those weren’t as impactful; no doubt Giordano and Nakanishi knew this when they formulated the Exercises.)

$~~~~$The other thing they no doubt knew when they formulated these Exercises is the "interesting-ness" of the region of the $F_D \times \Omega_D$, i.e., "parameter" space they assigned the student to explore; we couldn’t possibly describe here all $81$ results, but we will call attention to some "highlights." The first thing to investigate in a situation such as this is the effect of one parameter as the other remains fixed. First leaving $\Omega_D = 32/48 = 2/3$ fixed and scrolling through the options for $F_D$, we find that specific points of the attractor change, but the general shape does not&mdash;mathematically, all of these $F_D$, for this value of $\Omega_D$, are in the attractor’s [basin of attraction]()
...
<a name="Parker" onclick="window.history.back()">Parker</a>, T. S. and L. O. Chua, 1989, <i>Practical Numerical Algorithms for Chaotic Systems</i>, Springer-Verlag, New York, pg. 88.

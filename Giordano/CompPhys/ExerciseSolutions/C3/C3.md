...
$~~~\alpha$ even, $x_0$ (rational) $\in (0,0.5) \cup (0.5,1.0)$ appears to consistently result&mdash;in about 48 iterations (or half or a third of that) for Model a, and in four or fewer iterations for Model b&mdash;in a value of the form $1-k/2^j, j$ an integer, $k \in \{1, 2, ..., 2^{j-1}\}$, which gets sent to the fixed point 0 in $j$ steps.

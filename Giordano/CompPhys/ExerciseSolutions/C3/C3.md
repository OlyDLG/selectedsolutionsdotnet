...
$~~~~$We generalized the "models" to allow for bases, $b$, other than $b=2$ because, well, because it’s not hard to do and because we’re curious by nature. Thus the reader can see the first 64 iterates (the one parameter we have not made user-settable) for $b=2,3; n=5, 8, 11; \beta=0,1,2; \alpha \in \{1,2,...,15\};$ and $x_0 \in \{0.1, 0.2, ..., 0.9\}$. (Actually, "don’t tell anybody," but if you type in a different number in any of the numerical entry boxes and then "tab" away from the box&mdash;at least on Google Chrome on a Mac; don’t hit return, though, as that will reload the page and reset all parameters to their defaults&mdash;you may get a "complaint," but the program won’t crash and in fact it will perform the programmed iteration using the values in the boxes; thus you can actually study the first-fifty-iterates-behavior of these models for any floating-point values of the parameters you choose.) Also, in addition to graphs of the "time series"&mdash;$x_i$ vs. $i$&mdash;and the "next $x$ vs. current $x$"&mdash;$x_{i+1}$ vs. $x_i$&mdash;for each model, we provide a graph of $\Delta x_i = x_{i+1} - x_i$ vs. $x_i$&mdash;the discrete equivalent of plotting $\displaystyle \frac{dx}{dt}$ vs. $x$, i.e., the discrete "phase plane" graph

$~~~~$The reader will notice, however, some values omitted from the "approved choices"
    ...
    <tr style="border: none;">
      ...
      <td colspan=2 style="border: none;">
        <form id="C3E24x0" onchange="makeC3E24Graph()">
          ...
          <input type="number" name="C3E24x0In" min="0.1" max="0.9" step="0.1" value="0.5">
        </form>
      </td>
    </tr>
    <tr style="border: none; background-color: white;">

...
$~~~~$We generalized the "models" to allow for bases, $b$, other than $b=2$ because, well, because it’s not hard to do and because we’re curious by nature. Thus the reader can see the first 64 iterates (the one parameter we have not made user-settable; 64 was chosen because our investigations revealed other "long" cycles with periods equal to a power of 2, e.g., 16 and 32) for $b=2,3$; $n=5, 8, 11$; $\beta=0,1,2$; $\alpha \in \{1,2,...,15\}$; and $x_0 \in \{0.1, 0.2, ..., 0.9\}$. (Actually, "don’t tell anybody," but if you type in a different number in any of the numerical entry boxes and then "tab" away from the box&mdash;at least on Google Chrome on a Mac; don’t hit return, though, as that will reload the page and reset all parameters to their defaults&mdash;you may get a "complaint," but the program won’t crash and in fact it will perform the programmed iteration using the values in the boxes; thus you can actually study the first-sixty-four-iterates-behavior of these models for any floating-point values of the parameters you choose.)
 
$~~~~$Investigating $\beta=0$ first, we almost omitted $x_0=0$ from that parameter’s provided options, because when $\beta=0$, $x=0$ is a fixed point for any values of the other parameters; for Model a: $x_{i+1}(0) = \left[\alpha (0) b^n + 0\right] (\text{mod }b^n) / b^n = 0(\text{mod }b^n) /b^n = 0/b^n = 0$. Moreover, it is an [asymptotically stable](https://en.wikipedia.org/wiki/Lyapunov_stability#Definition_for_discrete-time_systems) fixed point: again working with Model a, choose integer $m$ such that $m \le \alpha x_i \lt m+1$, which is possible for any $\alpha, x_i$; this implies $0 \le \alpha x_i - m \lt 1 \Rightarrow$ $0 \le \alpha x_i b^n - mb^n \lt b^n \Rightarrow \left[\alpha x_i b^n\right](\text{mod }b^n) = \left[mb^n + (\alpha x_i b^n - mb^n)\right](\text{mod }b^n) = (\alpha x_i b^n - mb^n) \Rightarrow$ $\displaystyle \frac{x_{i+1}}{x_i} = \frac{\left[\alpha x_i b^n\right](\text{mod }b^n)/b^n}{x_i} = \frac{\alpha x_i - m}{x_i} = \alpha - \frac{m}{x_i}$ for the chosen $m$. Therefore, if there exists an $i$ such that $\displaystyle x_i \lt \delta = \frac{m}{\alpha-1}$ (note that this assumption compels us to treat $\alpha = 1$ as a separate case; the reader can either supply that proof or dismiss it as "uninteresting") $\displaystyle \Rightarrow \frac{x_i}{m} \lt \frac1{\alpha-1} \Rightarrow \alpha-1 \lt \frac{m}{x_i} \Rightarrow \alpha - \frac{m}{x_i} = \frac{x_{i+1}}{x_i} < 1 \Rightarrow x_{i+1} \lt x_i$. In words: there always exists a (positive) $\delta$&mdash;the value of which depends on $\alpha$, however: the larger $\alpha$ is, the smaller $x_i$ needs to be to be in $0$’s "basin of attraction"&mdash;such that if $x_i \lt \delta, x_{i+1} \lt x_{i}$ and the sequence will thenceforth monotonically approach, but never exactly equal, $0$, making $0$ asymptotically stable. As interesting as these results may (or may not) be, they only hold for $\beta=0$: even for $\alpha=1$, change $\beta$ to $1$ and $x_i=0$ is no longer a fixed point (though for $\beta=1, \alpha=2, b=2,$ and $n=8$, $x_i=1$ appears to be an "attracting" fixed point; establishing analytically whether it is or isn’t, we leave to the reader).
...
$~~~~$<h4>Model "b"</h4>
$\beta=0$, Base $=2$, $n=8$:<br>
<h5>Appears to be "good" for generating pseudo-random numbers<h5>
$\alpha=3, 5; x_0=0.1, 0.2, 0.6, 0.7$

<h5>Appears to have a period-8 cycle</h5>
$\alpha=7; x_0=0.3, 0.8<br>

<h5>Appears to have a period-16 cycle</h5>
$\alpha=3, 5; x_0=0.3, 0.8<br>
$\alpha=7; x_0=0.4, 0.9<br>

<h5>Appears to have a period-32 cycle</h5>
$\alpha=3, 5; x_0=0.4, 0.9<br>
$\alpha=7; x_0=0.1, 0.2, 0.6, 0.7<br>

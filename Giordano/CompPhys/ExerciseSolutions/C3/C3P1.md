...
When convenient, we use the &quot;over-dot&quot; notation to denote ordinary differentiation, e.g., $\dot{x} = \Frac{dx}{dt}, \ddot{x} = \Frac{d^2x}{dt^2}$, etc.
...
the following iteration produces a fourth-order-accurate numerical solution ($h = \Delta t, \u = \dot x$):

$$\begin{eqnarray}
g_i & = & x_n & + & c_i h \u_n + h^2\sum_{j=1}^{i-1}a_{ij}f(g_j) \\   
x_{n+1} & = & x_n & + & h\u_n + h^2\sum_{j=1}^3\bar{b}_jf(g_j) \\
\u_{n+1} & = & \u_n & + & h\sum_{j=1}^3b_jf(g_j) 
\end{eqnarray}
$$ where
...
$$\begin{eqnarray}
g_1 & = & x_n + \frac{3+\sqrt3}{6}h\u_n \\
g_2 & = & x_n + \frac{3-\sqrt3}{6}h\u_n + \frac{2-\sqrt3}{12}h^2f(g_1) \\
g_3 & = & x_n + \frac{3+\sqrt3}{6}h\u_n + \frac{\sqrt3}{6}h^2f(g_2) \\
x_{n+1} & = & x_n + h\u_n + h^2\left[\frac{5-3\sqrt3}{24}f(g_1) + \frac{3+\sqrt3}{12}f(g_2) + \frac{1+\sqrt3}{24}f(g_3)\right] \\
\u_{n+1} & = & \u_n + h\left[\frac{3-2\sqrt3}{12}f(g_1) + \frac12 f(g_2) + \frac{3+2\sqrt3}{12}f(g_3)\right] 
\end{eqnarray}
$$
...
$~~~~$(For convenience, henceforth, except in the graph labels, we use $x, \u$ in place of $\t, \omega$) For the physical pendulum model:
...
$~~~~$Of ccourse, the reader is encouraged to write their own program to investigate not only the effects of varying $F_D$ and $\Omega_D$, but also $q,~\ell,~\Delta t,$ and, especially, $x(0)$ and $\u(0)$.

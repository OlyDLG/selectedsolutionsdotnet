...
\dot{v} & = & -\frac{B_2}{m}\left|\vec{\upsilon}-\vec{W}\right|v - g, & v_0 = \upsilon_0\sin\theta \\
...
v_{n+1} & = & v_n - \left(\frac{{B_2}_{n}}{m}\left|\vec{\upsilon}_n-\vec{W}\right|v_n + g\right)\Delta t \\
...
$~~~~$Calculate the trajectory of a batted ball with side spin, i.e., spin parallel to the vertical axis.

### Solution
$~~~~$We could just use the model given in the text for a thrown ball with side spin, but in that model, drag was assumed negligible in the $y~\&~z$ directions, and spin was assumed negligible in the $x$ direction; however, we know from having solved Exercise 2.14 that, due to the larger "launch" angle and the much longer time-of-flight, these forces are less likely to be negligible; we will ignore wind in this model, however. 

$~~~~$Our continuous model is thus:

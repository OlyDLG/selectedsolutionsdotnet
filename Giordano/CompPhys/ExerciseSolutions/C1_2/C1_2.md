...
$~~~~$We are going to solve the general case, giving the reader five (horizontal) wind directions to choose from: tailwind, 45&deg; cross-tailwind, 90&deg; crosswind, 45&deg; cross-headwind, and headwind; $x$ continues to denote the (fixed) initial horizontal direction of the batted ball, $y$ continues to denote its altitude, and the new coordinate $z$&mdash;listed third in our ordered-triple vectors&mdash;denotes the horizontal orthogonal to $x$, i.e., the "lateral" displacement. The ball’s velocity vector (whose magnitude is denoted simply by $\upsilon$) is $\vec{\upsilon} = (u, v, w)$; the (constant) wind vector (speed $\omega$ user-selectable) is $\vec{\omega} = (\omega_x, 0, \omega_z) = (\omega\cos\phi, 0, \omega\sin\phi)$, $\phi$ such that a tailwind corresponds to $\phi=0$. The initial position $(x_0, y_0, z_0)$, in meters, is $(0, 1, 0)$; the initial velocity is $(\upsilon_0\cos\theta, \upsilon_0\sin\theta, 0)$, where $\upsilon_0$ is the initial speed (converted to meters per second from user-selectable miles per hour) and (user-selectable) $\theta$ is the "launch angle." For the time derivative we use Newton’s over-dot notation, e.g., $dx/dt = \dot{x}$. 
...
where $\displaystyle \frac{B_2}{m} = 0.0039 + \frac{0.0058}{1+\exp\left[\left(\left|\vec{\upsilon}-\vec{\omega}\right|-35\right)/5\right]}$ is given in the text. Discretized this becomes:
...
<center>
  <table style="width: 100%">
    <tr style="border: none; background: white">
      <td style="border: none;">
        <b>Flight time</b>: <span id="FlightTime"></span> seconds
      </td>
      <td style="border: none;">
        <b>Range</b>: <span id="Range"></span> meters (<span id="rangeFeet"></span> feet)
      </td>
      <td style="border: none;">
        <b>Lateral displacement</b>: <span id="DeltaZ"></span> meters (<span id="deltaZFeet"></span> feet)
      </td>
    </tr>
  </table>
</center>

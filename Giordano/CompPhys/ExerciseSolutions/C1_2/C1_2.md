...
<a name="Note" onclick="window.history.back()">\*</a>In order to highlight the effect of spin, we do not include wind in this Exercise; additionally, for the same purpose, we enable the reader to "turn off" (linear) drag in the lateral direction, despite that being "unphysical" (because without air resistance, spin does not give rise to a Magnus force).

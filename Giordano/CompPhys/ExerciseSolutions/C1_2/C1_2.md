...
$$N_A(t) = A_0 e^{-\alpha t}$$ 
where $A_0 = N_A(0)$ and $\alpha = \Frac1{\tau_A}$. (Check: $\Frac d{dt}A_0 e^{-\alpha t} = A_0(-\alpha) e^{-\alpha t} = -\alpha N_A = -N_A/\tau_A$; $N_A(0) = A_0 e^{-\alpha (0)} = A_0 \checkmark$.) We substitute this in for $N_A$ in Eq. 2 (and rename $\Frac1{\tau_B} = \beta$), yielding: 
...
\dot{y} & = & v, & y_0 = 0 \\
\dot{z} & = & w, & z_0 = 1 \\
\dot{u} & = & -\frac{B_2}{m}\u u - \frac{S_0}{m}(\vec{\omega} \times \vec{\u})_x, & u_0 = \u_0\cos\t \\
\dot{v} & = & -\frac{B_2}{m}\u v - \frac{S_0}{m}(\vec{\omega} \times \vec{\u})_y, & v_0 = 0 \\
\dot{w} & = & -\frac{B_2}{m}\u w - g, & w_0 = \u_0\sin\t \\
\end{eqnarray}
$$

where, as in the text, $\Frac{S_0}{m} = 4.1\times 10^{-4}, \Frac{B_2}{m}$ is as in Exercise 2.14, and $\vec{\omega} = (0,0,\omega), \omega$ user-selectable. Discretized, this becomes:

$$\begin{eqnarray}
x_{n+1} & = & x_n + u_n\Delta t \\
y_{n+1} & = & y_n + v_n\Delta t \\
z_{n+1} & = & z_n + w_n\Delta t \\
u_{n+1} & = & u_n -\left(\frac{{B_2}_{n}}{m}\u_n u_n + \frac{S_0}{m}(\vec{\omega} \times \vec{\u}_n)_x\right)\Delta t \\
v_{n+1} & = & v_n - \left(\frac{{B_2}_{n}}{m}\u_n v_n + \frac{S_0}{m}(\vec{\omega} \times \vec{\u}_n)_y\right)\Delta t \\

// Giordano/Nakanishi Comp. Phys. Chpt. 1 Ex. 4 & 5

const n1 = 100, sn1 = String(n1); 

function getn2(altNoIntvlsForm) {
    let n2 = altNoIntvlsForm[0].valueAsNumber;
    return n2;
}
...
function addExact(Exact, data, tauA, tauB, NA0, NB0, tMax) {
    for (i=0; i < n1+1; i++) {
        let t = dt(tMax, n1) * i;
        let [N_A, N_B] = Exact(t, tauA, tauB, NA0, NB0);
...
function GiorCPC1E4() { // Giordano/Nakanishi Comp. Phys. Chpt. 1 Ex. 4 chart
    ...
    function Exact(t, a, b, A0, B0) {
        let N_A = A0 * Math.exp(-a * t);
        let N_B = (a == b) ? (B0 + A0 * a * t) * Math.exp(-a * t) 
                           : B0 * Math.exp(-b * t) + A0 / (b/a - 1) * (Math.exp(-a * t) - Math.exp(-b * t));
        return [N_A, N_B];
    }
...
// Add n2 intervals Euler solution
        ...
        let title = "<center><h3>N<sub>A</sub> and N<sub>B</sub> vs. Time: ";
        ...
        let options = {...globalChartOptions,
                       ...
                       series: {2: {lineWidth: 0, pointSize: 4, pointShape: 'square'},
        ...
        let chart = new google.visualization.LineChart(document.getElementById("GiorCPC1E4chart"));
...
function GiorCPC1E5() { // Giordano/Nakanishi Comp. Phys. Chpt. 1 Ex. 5 chart
    ...
    function Exact(t, tau, tau, A0, B0) {
        let N_A = ( (A0-B0) * Math.exp(-2 * t) + A0 + B0 ) / 2.0;
        let N_B = A0 + B0 - N_A;
        return [N_A, N_B];
    }
    ...
    // Add n2 intervals Euler solution
      ...
      // Display the chart inside the <sp> element with id="GiorCPC1E5chart"
        let chart = new google.visualization.LineChart(document.getElementById("GiorCPC1E5chart"));
        chart.draw(dtable, options);

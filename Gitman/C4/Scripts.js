function G4P2() {
  const FVIF1 = FutValIntFac(Number(dgEBIiH("P2r1"))/100, Number(dgEBIiH("P2n1")));
  dsEBIiH("P2f1", "<sp class=sum>" + FVIF1.toFixed(3) + "</sp>");

  const FVIF2 = FutValIntFac(Number(dgEBIiH("P2r2"))/100, Number(dgEBIiH("P2n2")));
  dsEBIiH("P2f2", "<sp class=sum>" + FVIF2.toFixed(3) + "</sp>");
  
  const FVIF3 = FutValIntFac(Number(dgEBIiH("P2r3"))/100, Number(dgEBIiH("P2n3")));
  dsEBIiH("P2f3", "<sp class=sum>" + FVIF3.toFixed(3) + "</sp>");
  
  const FVIF4 = FutValIntFac(Number(dgEBIiH("P2r4"))/100, Number(dgEBIiH("P2n4")));
  dsEBIiH("P2f4", "<sp class=sum>" + FVIF4.toFixed(3) + "</sp>");
}

function G4P3() {
  const r1 = Number(dgEBIiH("P3r1"))/100;
  const DT1 = FutValn(1, 2, r1);
  dsEBIiH("P3D1", "<sp class=sum>" + DT1.toFixed(1) + "</sp>");
  const QT1 = FutValn(1, 4, r1);
  dsEBIiH("P3Q1", "<sp class=sum>" + QT1.toFixed(1) + "</sp>");

  const r2 = Number(dgEBIiH("P3r2"))/100;
  const DT2 = FutValn(1, 2, r2);
  dsEBIiH("P3D2", "<sp class=sum>" + DT2.toFixed(1) + "</sp>");
  const QT2 = FutValn(1, 4, r2);
  dsEBIiH("P3Q2", "<sp class=sum>" + QT2.toFixed(1) + "</sp>");

  const r3 = Number(dgEBIiH("P3r3"))/100;
  const DT3 = FutValn(1, 2, r3);
  dsEBIiH("P3D3", "<sp class=sum>" + DT3.toFixed(1) + "</sp>");
  const QT3 = FutValn(1, 4, r3);
  dsEBIiH("P3Q3", "<sp class=sum>" + QT3.toFixed(1) + "</sp>");

  const r4 = Number(dgEBIiH("P3r4"))/100;
  const DT4 = FutValn(1, 2, r4);
  dsEBIiH("P3D4", "<sp class=sum>" + DT4.toFixed(1) + "</sp>");
  const QT4 = FutValn(1, 4, r4);
  dsEBIiH("P3Q4", "<sp class=sum>" + QT4.toFixed(1) + "</sp>");
}

function G4P5() {
    const r = Number(dgEBIiH("P5r")) / 100;
    const y3 = Number(dgEBIiH("P5Y3"));
    const Y0B = Number(dgEBIiH("P5B0"));
    const Y3B = FutVal(Y0B, r, y3);
    dsEBIiH("P5B3", "<sp class=sum>" + addCommas(Y3B.toFixed(2)) + "</sp>");    
    dsEBIiH("P5D3", "<sp class=sum>" + (Y3B - Y0B).toFixed(2) + "</sp>");    
    const Y6B = FutVal(Y0B, r, 2*y3);
    dsEBIiH("P5B6", "<sp class=sum>" + addCommas(Y6B.toFixed(2)) + "</sp>");    
    dsEBIiH("P5D6", "<sp class=sum>" + (Y6B - Y3B).toFixed(2) + "</sp>");    
    const Y9B = FutVal(Y0B, r, 3*y3);
    dsEBIiH("P5B9", "<sp class=sum>" + addCommas(Y9B.toFixed(2)) + "</sp>");
    dsEBIiH("P5D9", "<sp class=sum>" + (Y9B - Y6B).toFixed(2) + "</sp>");    
}

function G4P9() {
  const PVIF1 = 1 / FutValIntFac(Number(dgEBIiH("P9r1"))/100, Number(dgEBIiH("P9n1")));
  dsEBIiH("P9f1", "<sp class=sum>" + PVIF1.toFixed(3) + "</sp>");

  const PVIF2 = 1 / FutValIntFac(Number(dgEBIiH("P9r2"))/100, Number(dgEBIiH("P9n2")));
  dsEBIiH("P9f2", "<sp class=sum>" + PVIF2.toFixed(3) + "</sp>");
  
  const PVIF3 = 1 / FutValIntFac(Number(dgEBIiH("P9r3"))/100, Number(dgEBIiH("P9n3")));
  dsEBIiH("P9f3", "<sp class=sum>" + PVIF3.toFixed(3) + "</sp>");
  
  const PVIF4 = 1 / FutValIntFac(Number(dgEBIiH("P9r4"))/100, Number(dgEBIiH("P9n4")));
  dsEBIiH("P9f4", "<sp class=sum>" + PVIF4.toFixed(3) + "</sp>");
}

function G4P14() {
  const n = [Number(dgEBIiH("P14n1")), Number(dgEBIiH("P14n2"))];
  const r = [Number(dgEBIiH("P14r1"))/100, Number(dgEBIiH("P14r2"))/100, Number(dgEBIiH("P14r3"))/100];
  const cells = [["P14f11", "P14f12"], ["P14f21", "P14f22"], ["P14f31", "P14f32"]];
...
function G4P28() {
  const r = Number(dgEBIiH("P28r")) /100;
  let pvif, m, pv, I;//, idx;
  let Asum = 0, Bsum = 0;
  for (i=1; i < 6; i++) {
    I = i.toFixed(0);
    pvif = 1/FutValIntFac(r, i);
    dsEBIiH("P28PVIF" + I, "<sp class=sum>" + pvif.toFixed(3) + "</sp>");
    m = Number(removeCommas(dgEBIiH("P28CFA" + I)));
    pv = pvif * m;
    dsEBIiH("P28PVA" + I, "<sp class=sum>" + addCommas(pv.toFixed(2)) + "</sp>");
    Asum += pv;
    m = Number(removeCommas(dgEBIiH("P28CFB" + I)));
    pv = pvif * m;
    dsEBIiH("P28PVB" + I, "<sp class=sum>" + addCommas(pv.toFixed(2)) + "</sp>");
    Bsum += pv;    
  }

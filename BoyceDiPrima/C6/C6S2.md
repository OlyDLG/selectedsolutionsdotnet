...
<table>
  <tr>
    ...
    <td><a href="#P5">Problem 5</a></td>
    <td><a href="#P8">Problem 8</a></td>
    <td><a href="#P11">Problem 11</a></td>
    <td><a href="#P14">Problem 14</a></td>
  </tr>
  <tr>
    <td><a href="#P17">Problem 17</a></td>
    <td><a href="#P20">Problem 20</a></td>
    <td><a href="#P23">Problem 23</a></td>
    <td><a href="#P26">Problem 26</a></td>
    <td><a href="#P27">Problem 27</a></td>
  </tr>
  <tr>
    <td><a href="#P30">Problem 30</a></td>
    <td><a href="#P33">Problem 33</a></td>
    <td><a href="#P36">Problem 36</a></td>
  </tr>
</table>
</div>
...
<b>Sln</b>: $\mathcal{L}^{-1}\left[\Frac{4}{(s-1)^3}\right] = 2\mathcal{L}^{-1}\left[\Frac{n!}{(s-a)^{n+1}}\right]$ where $n=2, a=1, \underset{\text{TE 11}}{=} \box{2t^2e^t}$
<br><br>

<a name="P5" class="goback" onclick="winhisback()">__5__)</a> 
$\displaystyle \mathcal{F}(s) = \frac{2s+2}{s^2 + 2s + 5}$

<b>Sln</b>: $\mathcal{L}^{-1}\left[\Frac{2s+2}{s^2 + 2s + 5}\right] = 2\mathcal{L}^{-1}\left[\Frac{s-(-1)}{(s-(-1))^2 + 2^2}\right] = 2\mathcal{L}^{-1}\left[\Frac{s-a}{(s-a)^2 + b^2}\right]$ where $a=-1, b=2, \underset{\text{TE 10}}{=} \box{2e^{-t}\cos(2t)}$
<br><br>
...
$\begin{eqnarray} 
\textbf{Sln}\text{: }\mathcal{L}^{-1}\left[\Frac{8s^2 - 4s + 12}{s(s^2+4)}\right] & = & \mathcal{L}^{-1}\left[\Frac{A}s + \frac{Bs+C}{s^2+4}\right], \text{where } A+B = 8, C = -4, 4A = 12 \implies A = 3, B = 5, \\
& = & 3\mathcal{L}^{-1}\left[\Frac1{s}\right] + 5\mathcal{L}^{-1}\left[\frac{s}{s^2+2^2}\right] - 2\mathcal{L}^{-1}\left[\frac{2}{s^2+2^2} \right] \\
& \underset{\text{TE 1, 6, 5}}{=} & \box{3 + 5\cos(2t) - 2\sin(2t)}
\end{eqnarray}$
<br><br>
...
$\begin{eqnarray} \implies \mathcal{L}[y] = \Frac{s-2}{s^2 - s - 6} \implies y = \mathcal{L}^{-1}\left[\Frac{s-2}{s^2 - s - 6}\right] & = & \frac15\mathcal{L}^{-1}\left[\frac{1}{s-3}\right] + \frac45\mathcal{L}^{-1}\left[\frac{1}{s+2}\right] \\
& \underset{\text{TE 2}}{=} & \box{\frac{e^{3t} + 4e^{-2t}}{5}}
\end{eqnarray}$

$\chec: y(0) = \frac{1+4}{5} = 1~\chec \\
y’ = \Frac{3e^{3t} - 8e^{-2t}}{5} \implies y’(0) = \frac{3-8}{5} = -1~\chec \\
y’’ = \Frac{9e^{3t} + 16e^{-2t}}{5} \implies y’’ - y’ - 6y = \frac{(9-3-6)e^{3t} + (16 - (-8) - 24)e^{-2t}}{5} = 0~\chec$
<br><br>
...
$\begin{eqnarray} \implies \mathcal{L}[y] = \Frac{s-3}{(s-2)^2} \implies y & = & \mathcal{L}^{-1}\left[\Frac{1}{s-2}\right] - \mathcal{L}^{-1}\left[\frac1{(s-2)^2}\right] \\
& \underset{\text{TE 2 & 11}}= & e^{2t} - te^{2t} = \box{(1-t)e^{2t}} 
\end{eqnarray}$

$\chec : y(0) = (1-0)e^{2(0)} = 1~\chec \\
y’ = 2(1-t)e^{2t} - e^{2t} = (1-2t)e^{2t} \implies y’(0) = (1-0)e^0 = 1~\chec \\
y’’ = 2(1-2t)e^{2t} - 2e^{2t} = -4te^{2t} \implies y’’ - 4y’ + 4y = -4te^{2t} - 4(1-2t)e^{2t} + 4(1-t)e^{2t} = (-4t + 8t - 4t - 4 + 4)e^{2t} = 0~\chec
$<br><br>
...
$\begin{eqnarray} \implies \mathcal{L}[y] = \Frac{s^2 - 4s + 7}{(s-1)^4} \implies y & = & \mathcal{L}^{-1}\left[\Frac{0}{s-1}\right] + \mathcal{L}^{-1}\left[\frac{1}{(s-1)^2}\right] + \mathcal{L}^{-1}\left[\frac{-2}{(s-1)^3}\right] + \mathcal{L}^{-1}\left[\frac{4}{(s-1)^4}\right] \\
...
& \underset{\text{TE 11}}{=} & te^t - t^2e^t +\frac23 t^3e^t = \box{\left(\frac23 t^3 - t^2 + t\right)e^t}
\end{eqnarray}$

$\chec$-step: Left for the reader
<br><br>
...
$\begin{eqnarray} 
\textbf{Sln}\text{: }\mathcal{L}[\cos(2t)] \underset{\text{TE 6}}{=} \frac{s}{s^2 + 4} & = & \mathcal{L}\left[y’’ + \w^2y\right] \\
& = & [s^2\mathcal{L}[y] - sy(0) - y’(0)] + \w^2\mathcal{L}[y] \\
& = & (s^2 + \w^2)\mathcal{L}[y] - s \\
\end{eqnarray}$

$\implies \mathcal{L}[y] = \Frac{s^3 + 5s}{(s^2+4)(s^2+\w^2)} \underset{\because~\w^2 \ne 4}{=} \left(\frac1{\w^2-4}\right)\frac{s}{s^2+4} + \left(\frac{\w^2-5}{\w^2-4}\right)\frac{s}{s^2+\w^2}$

$\begin{eqnarray}
\implies y & = & \displaystyle \left(\frac1{\w^2-4}\right)\mathcal{L}^{-1}\left[\frac{s}{s^2+4}\right] + \left(\frac{\w^2-5}{\w^2-4}\right)\mathcal{L}^{-1}\left[\frac{s}{s^2+\w^2}\right] \\
& \underset{\text{TE 6}}{=} & \displaystyle \box{\left(\frac1{\w^2-4}\right)\cos(2t) + \left(\frac{\w^2-5}{\w^2-4}\right)\cos(\w t)} \\
\end{eqnarray}$

$\chec: y(0) = \displaystyle \left(\frac1{\w^2-4}\right) + \left(\frac{\w^2-5}{\w^2-4}\right) = \frac{\w^2-4}{\w^2-4} = 1~~\chec \\
y’(0) = -2 \displaystyle \left(\frac1{\w^2-4}\right)\sin(2(0)) - \w\left(\frac{\w^2-5}{\w^2-4}\right)\sin(\w(0)) = 0 + 0 = 0 ~~\chec \\
\begin{eqnarray}
y’’+ \w^2 y & = & -4\displaystyle \left(\frac1{\w^2-4}\right)\cos(2t) - \cancel{\w^2\left(\frac{\w^2-5}{\w^2-4}\right)\cos(\w t)} + \w^2 \displaystyle \left(\frac1{\w^2-4}\right)\cos(2t) + \cancel{\w^2\left(\frac{\w^2-5}{\w^2-4}\right)\cos(\w t)} \\
& = & (\w^2-4)\displaystyle \left(\frac1{\w^2-4}\right)\cos(2t) \\
& = & \cos(2t)~~\chec
\end{eqnarray}$
<br><br>
...
$\implies \mathcal{L}[y] = \Frac4{(s+1)^3} + \frac{2s+3}{(s+1)^2} = \frac{2s^2 + 5s + 7}{(s+1)^3} = \frac2{s+1} + \frac1{(s+1)^2} + \frac4{(s+1)^3}
$
...
$\chec: y(0) = (2 + 0 + 2(0)^2)e^{-0} = 2~~\chec \\
y’(t) = (1+4t)e^{-t} - (2 + t + 2t^2)e^{-t} = (-1 + 3t - 2t^2)e^{-t} \implies y’(0) = -1~~\chec \\
y’’ = (3-4t)e^{-t} - (-1 + 3t - 2t^2)e^{-t} = (4 - 7t + 2t^2)e^{-t} \implies \\
y’’ + 2y’ + y = [4 - 7t + 2t^2 + 2(-1 + 3t - 2t^2) + 2 + t + 2t^2]e^{-t} = [(6-2) + (7-7)t + (4-4)t^2]e^{-t} = 4e^{-t}~~\chec$
<br><br>
...
We thus have $(s^2+4)\mathcal{L}[y] = \Frac{1-e^{-s}}{s^2} \implies \box{\mathcal{L}[y] = \frac{1-e^{-s}}{s^2(s^2+4)}}$
<br><br>
...
<b>Sln</b>: The T. S. [about 0] of $\sin t = \displaystyle \sum_{n=0}^{\infty} \frac{(-1)^n t^{2n+1}}{(2n+1)!} \implies$<br>$\mathcal{L}[\sin t] = \displaystyle \sum_{n=0}^{\infty} \frac{(-1)^n}{(2n+1)!}\mathcal{L}[t^{2n+1}] = \sum_{n=0}^{\infty} \frac{(-1)^n}{\cancel{(2n+1)!}}\frac{\cancel{(2n+1)!}}{s^{2n+2}} = \frac1{s^2}\sum_{n=0}^{\infty} \left(-\frac1{s^2}\right)^n = \frac1{s^2}\frac1{1-(-1/s^2)}$ (because the sum is that of a geometric series of constant term 1 and common ratio $-1/s^2$, the sum of which is well-known to be $[1-(-1/s^2)]^{-1}$) $= \box{\frac1{s^2 + 1}}$ 
<br>
...
$\displaystyle \mathcal{L}[J_0(\sqrt t)]  =  \sum_{n=0}^{\infty}\frac{(-1)^n}{2^{2n}(n!)^2} \mathcal{L}[t^n]  \underset{\text{TE 3}}{=} \sum_{n=0}^{\infty}\frac{(-1)^n}{2^{2n}(n!)^2} \frac{n!}{s^{n+1}} = \frac1s \sum_{n=0}^{\infty}\frac{(-1)^n}{2^{2n}n!s^n}  = \frac1s \sum_{n=0}^{\infty}\frac{[1/(-4s)]^n}{n!} = \box{\frac{e^{-1/(4s)}}{s}}$
...
<b>Sln</b>: $t^2\sin bt = (-t)^2f(t)$ where $f(t) = \sin bt \therefore$
 ...
 $\mathcal{L}[t^2\sin bt] = (\mathcal{L}[\sin bt])’’ = \displaystyle \left(\frac{b}{s^2+b^2}\right)’’ = b\left[-2s(s^2+b^2)^{-2}\right]’ = -2b\left[(s^2+b^2)^{-2} - 4s^2(s^2+b^2)^{-3}\right] =$ $$\box{\frac{2b(3s^2-b^2)}{(s^2+b^2)^3}}$$
<br><br>
...
$\mathcal{L}[te^{at}\sin bt] = -(\mathcal{L}[e^{at}\sin bt])’ = \displaystyle -\left(\frac{b}{(s-a)^2 + b^2}\right)’ = -2(-b)((s-a)^2 + b^2)^{-2}(s-a) = \box{\frac{2b(s-a)}{\left[(s-a)^2+b^2\right]^2}}$ 
<br><br>
...
<b>Sln</b>: $\mathcal{L}[0] = 0 = \mathcal{L}[y’’ - ty] = s^2Y - sy(0) - y’(0) + Y’ = Y’ + s^2Y - s \implies \box{Y’ +s ^2Y = s}$
<br><br>
...
\box{s^2Y’’ + 2sY’ - [s^2+\alpha(\alpha+1)]Y = -1}$
<br><br>

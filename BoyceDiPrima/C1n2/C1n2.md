...
$\Frac{x}{dx} = \Frac1{d\u}\left(\Frac{\u-4}{1-\u} - \u\right) = \Frac1{d\u}\left(\Frac{\u^2-4}{1-\u}\right) \implies 0 = \Frac{dx}x + \Frac{\u-1}{(\u+2)(\u-2)}d\u = \Frac{dx}x +\Frac3{4(\u+2)}d\u + \Frac1{4(\u-2)}d\u \implies$
$4\log|x| + 3\log|\u + 2| + \log|\u-2| = c \implies \cancel{x^4}\Frac{|y+2x|^3}{\cancel{|x|^3}}\Frac{|y-2x|}{\cancel{|x|}} = C \implies$ the solution is
$$\box{|y-2x||y+2x|^3 = C, x\ne 0}$$
(It’s not clear if we ___have___ to omit $x = 0$ from the domain of the solution, but our solution method precluded it, so we exclude it just to be safe.)<br><br>

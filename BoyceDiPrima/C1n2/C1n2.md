...
__Sln__: Noting that $1+x^2 \ne 0$ for all real $x$, divide through by it to yield the equivalent DE $y’ + 4x(1+x^2)^{-1}y = (1+x^2)^{-3} = y’ + p(x)y = g(x)$ with $p(x) = 4x(1+x^2)^{-1}, g(x) = (1+x^2)^{-3}$.&nbsp; Thus the integrating factor $ \mu(x) = \exp\left(\int\Frac{4x}{1+x^2}dx\right) = $ (letting $u = 1+x^2 \implies du = 2xdx$)<br> 
$ \exp\left(2\int\Frac{du}{u}\right) = e^{2\log u} = (1+x^2)^2$.&nbsp; Multiplying the $y’ + p(x)y = g(x)$ form of the equation through by that yields:<br>

...
<span class="goback">Purple</span> font indicates clicking on the text will return you to your prior place, $\blacksquare$ inidicates the end of a proof.
...
__Sln__: (I prefer to solve these using the method rather than the formula.)&nbsp; This is of the form $y’ + p(x)y = g(x)$ with $p(x) = 1/x, g(x) = 3\cos 2x$; thus the integrating factor $\displaystyle \mu(x) = \exp\left(\int \frac1x dx\right) = e^{\log x} = x$.&nbsp; Multiplying through by $x$ (which we can do without concern since it’s stipulated that $x \gt 0$), we get:<br>

$\displaystyle xy’ + y = \frac d{dx}(xy) = 3x\cos 2x \implies xy = \int 3x\cos 2x dx = \frac34\left(2x\sin 2x + \cos 2x\right) + C \implies$ 

$$\boxed{y = \frac32\sin 2x + \frac{3\cos 2x}{4x} + \frac Cx}$$
Check: $\displaystyle y’ = \frac32 (2)\cos 2x + \frac{4x(-6\sin 2x) - 4(3\cos 2x)}{16x^2} - \frac C{x^2} = 3\cos 2x - \frac{3\sin 2x}{2x} - \frac{3\cos 2x}{4x^2} - \frac C{x^2}$,<br>
$\displaystyle (1/x)y = \frac{3\sin 2x}{2x} + \frac{3\cos 2x}{4x^2} + \frac C{x^2}$ so<br>
$\displaystyle y’ + (1/x) = 3\cos 2x \cancel{- \frac{3\sin 2x}{2x} + \frac{3\sin 2x}{2x}} \cancel{- \frac{3\cos 2x}{4x^2} + \frac{3\cos 2x}{4x^2}} \cancel{- \frac C{x^2} + \frac C{x^2}}.~~\checkmark$
<br><br>

<a name="P2.1.8" class="goback" onclick="winhisback()">__2.1.8__)</a>
$(1+x^2)y’ + 4xy = (1+x^2)^{-2}$

__Sln__: Noting that $1+x^2 \ne 0$ for all real $x$, divide through by it to yield the equivalent DE $y’ + 4x(1+x^2)^{-1}y = (1+x^2)^{-3} = y’ + p(x)y = g(x)$ with $p(x) = 4x(1+x^2)^{-1}, g(x) = (1+x^2)^{-3}$.&nbsp; Thus the integrating factor $\displaystyle \mu(x) = \exp\left(\int\frac{4x}{1+x^2}dx\right) = $ (letting $u = 1+x^2 \implies du = 2xdx$)<br> 
$\displaystyle \exp\left(2\int\frac{du}{u}\right) = e^{2\log u} = (1+x^2)^2$.&nbsp; Multiplying the $y’ + p(x)y = g(x)$ form of the equation through by that yields:<br>
$\displaystyle (1+x^2)^2 y’ + 4x(1+x^2)y = \frac d{dx}\left[(1+x^2)^2 y\right] = \frac1{1+x^2} \implies (1+x^2)^2y = \int\frac1{1+x^2}dx = \tan^{-1}x + C \implies$
$$\boxed{y = \frac{\tan^{-1}x + C}{(1+x^2)^2}}$$
Check: $\displaystyle (1+x^2)y’ = (1+x^2)\left[\frac{(1+x^2)^2(1+x^2)^{-1} - 4x(1+x^2)(\tan^{-1}x + C)}{(1+x^2)^4}\right] = (1+x^2)^{-2}(1 - 4x(\tan^{-1}x + C)$<br>
...
$\displaystyle \lim_{x\rightarrow\pm\infty} \frac{\tan^{-1}x + C}{(1+x^2)^2} \rightarrow \frac{\pm\frac{\pi}2 + C}{\infty} \rightarrow 0$.
<br><br>

...
A skydiver weighing 180 lb falls falls from an altitude $x = 5000$ ft and opens the parachute after 10 sec of free fall; assume that the force of air resistance is $0.75|\u|$ when the parachute is closed and $12|\u|$ when the parachute is open ($\u$ in ft/sec).&nbsp; __a__) What is $|\u|$ when the parachute opens?&nbsp; __b__) What is the distance fallen when the parachute opens?&nbsp; __c__) What is the limiting velocity $\u_{\ell}$ after the parachute opens?&nbsp; __d__) How long (approximately) is the skydiver in the air after the parachute opens?

__Sln__: The basic governing equations are the same for both parts of the motion, all that changes are the values of the friction coefficient and the initial conditions.&nbsp; Accordingly, we will obtain formulas for the required velocities and displacements by solving the models symbolically, and then plug in numerical values after we have checked our solutions’ general veracity.

Choosing down to be the positive direction, weight, $w = mg$, has a positive sign, whereas air resistance $= k|\u|$ in magnitude ($k = 0.75$ for parts a) and b), $12$ for parts c) and d) will act in the opposite direction and thus the model for velocity, $\u$, is: 

$$m\frac{d\u}{dt} = mg - k\u \implies \frac{d\u}{dt} = g - \frac km\u \implies \frac{d\u}{dt} + \frac km \u = g~~~~~~~~~~\text{(Eq. 1)}$$

Setting $\a = \Frac km$, an integrating factor for Eq. 1 is $\mu(t) = \exp\left(\Int \a dt\right) = e^{\a t}$ and multiplying Eq. 1 through by $\mu(t)$ yields:

$$e^{\a t}\frac{d\u}{dt} + \a e^{\a t}\u = \frac d{dt}\left(e^{\a t}\u\right) = ge^{\a t} \implies e^{\a t}\u = \frac g{\a} e^{\a t} + C_{\u} \implies \box{\u = \frac g{\a} + C_{\u}e^{-\a t}}$$

Setting $\u(0) = \u_0$, we have $\u_0 = \Frac g{\a} + C_{\u}e^{(-\a)(0)} \implies \box{C_{\u} = \u_0 - \Frac g{\a}}.~$ Check: IC: $\u(0) = \Frac g{\a} + \left(\u_0 - \Frac g{\a}\right)e^{(-\a)(0)} = \u_0~\chec$; DE: $\Frac{d\u}{dt} + \a\u = -\a C_{\u}e^{-\a t} + \a\left(\Frac g\a + C_{\u}e^{-\a t}\right) = g.~\chec$

To obtain $x(t)$, we simply set $\Frac{dx}{dt} = \u(t) = \Frac g{\a} + C_{\u}e^{-\a t} \implies x = \Int\left(\Frac g{\a} + C_{\u}e^{-\a t}\right)dt = \box{\Frac g{\a} t - \Frac{C_{\u}}\a e^{-\a t} + C_x}$; setting $x(0) = x_0 = \Frac g{\a} (0) - \Frac{C_{\u}}{\a} e^{(-\a)(0)} + C_x = -\Frac{C_{\u}}\a + C_x \implies \box{C_x = x_0 + \frac{C_{\u}}{\a}}.~$ Check: IC: $x(0) = \Frac g{\a} (0) - \Frac{C_{\u}}{\a} e^{(-\a)(0)} + x_0 + \Frac{C_{\u}}{\a} = x_0~\chec$; DE: $\Frac{dx}{dt} = \Frac g{\a} -(-\a)\Frac{C_{\u}}{\a} e^{-\a t} = \Frac g{\a} + C_{\u}e^{-\a t} = \u(t).\chec$&nbsp; Now it’s time to plug in some numbers!

__a__) We need to evaluate $\u(10)$ given $\u_0 = 0, \a = \Frac km = \Frac k{w/g} = \Frac{gk}w = \Frac{32(0.75)}{180} = \Frac2{15} \implies \Frac g{\a} = \Frac{gw}{gk} = \Frac wk = \Frac{180}{0.75} = 240 \implies$ 
$\u(10) = 240\left(1 - \exp\left[-\Frac2{15}(10)\right]\right) = 240(1-e^{-4/3}) \doteq \box{176.7\text{ ft/sec}}$

__b__) Since we’re asked for the distance fallen, as opposed to the altitude, we can choose $x_0 = 0 \implies$ $x(10) = \Frac g{\a}(10) + \Frac g{\a^2}e^{-4/3} - \Frac g{\a^2} = 2400 + \Frac{240}{2/15}\left(e^{-4/3} - 1\right) \doteq \box{1074.5\text{ ft}}$

__c__) After the parachute opens the velocity model is $\u(t) = \Frac g{\a} + \left(176.7 - \Frac g{\a}\right)e^{-\a t}$, where $\a = \Frac{32(12)}{180} = \Frac{32}{15} \implies \Frac g{\a} = 15$, and as $t \rarrow \infty,~e^{-\a t} \rarrow 0$ so the &quot;terminal&quot; velocity $\box{\u_{\ell} = \frac g{\a} = 15\text{ ft/sec}}$

__d__) Resetting $x_0 = 0$, we need to find $t$ such that $x(t) = 15t + \Frac{176.7 - 15}{32/15}[1 - \exp(-32t/15)] = 5000 - 1074.5 = 3925.5$ ft.&nbsp; It is not possible to solve this equation explicitly for $t$, so we must use an approximation procedure; fortunately, nowadays (2026), this is trivial using pretty much any graphing utility or calculator.&nbsp; Even so, there are still a few different ways to go about it, e.g., find the intersection of $x(t)$ and the horizontal line $x = 3925.5$; or the intersection of $f(t) = 3925.5 - 15t$ and $g(t) = \Frac{176.7 - 15}{32/15}[1 - \exp(-32t/15)]$; or&mdash;the way that I prefer, because it requires you to define and plot only one function&mdash;the zero of $h(t) = 3925.5 - x(t)$.&nbsp; This graph and its zero are plotted below, from which we see that the time aloft after the parachute has been deployed is approximately $\box{256.6\text{ sec}}.$&nbsp; (We also see that the skydiver reaches the new terminal velocity VERY quickly, compared to the total time of fall, as evidenced by the very small interval of time after $t=0$ during which the curve is non-linear, compared to the almost entire time aloft during which the displacement is imperceptibly different from linear vs. time.)<br>
 
<div class="tqimg"><img src="./Graphs/BoyceDiPrimaC2S7P7.png"></div>
<br>
...
__Sln__: Taking &quot;down&quot; to be the positive direction, the governing model is:

$$m\frac{d\u}{dt} = mg - k\u^r \implies \frac{d\u}{dt} = g - \Frac km \u^r$$

where $k \gt 0$ is the constant of proportionality.&nbsp; Unless $r=1$ (in which case the equation can be solved exactly using the integrating factor technique), this is a first&#8209;order, nonlinear equation of a type we have not been taught how to solve (it is not, e.g., of the Bernoulli type, covered in the Section 2 exercises).&nbsp; So what methods have we learned to determine the limiting behavior of a model when an exact solution isn’t available to us?&nbsp; We could construct the direction field for a variety of values of the parameters and see if we notice any patterns; that’s precisely what I did, and graphing just one such direction field was sufficient to &quot;remind&quot; me: ___the limiting value must be a stable equilibrium of the model.___&nbsp; (This is a general principle the reader needs to learn and remember!)

So, set $0 = g - \Frac km \u^r \implies$ $$\box{\u_{\ell} = \left(\frac{mg}k\right)^{1/r}}$$

(Of course, we haven’t proven that this is a ___stable___ equilibrium: we leave that for the reader.)
<br><br>
...
__Sln__: The mass of the drop is its volume, $\Frac43\pi a^3$ times its density, $\rho$, while the mass of the fluid it displaces is the same volume times its density, $\rho’$; therefore, again assuming down = positive displacement, the force balance gives $$m\frac{d\u}{dt} = mg - \frac43\pi a^3\rho’g - 6\pi\mu a\u \implies \frac{d\u}{dt} = g\left(1-\frac{\rho’\cancel{4\pi a^3/3}}{\rho\cancel{4\pi a^3/3}}\right) - \frac{6\pi\mu a\u}{\rho 4\pi a^3/3} = g\left(1-\frac{\rho’}{\rho}\right) -\frac{9\mu}{2\rho a^2}\u.$$
The limiting velocity, $\u_{\ell}$, is the model’s equilibrium, obtained by setting $\Frac{d\u}{dt} = 0 = g\left(1-\frac{\rho’}{\rho}\right) -\frac{9\mu}{2\rho a^2}\u_{\ell} \implies \frac{9\mu}{2a^2}\u_{\ell} = g(\rho - \rho’) \implies$
$$\box{\u_{\ell} = \frac{2a^2g(\rho - \rho’)}{9\mu}}$$ 
...
__Sln__: The model is now $$m\frac{d\u}{dt} = mg - \frac43\pi a^3\rho’g - 6\pi\mu a\u - Ee \implies \frac{d\u}{dt} = g\left(1 - \frac{\rho’}{\rho}\right) - 6\pi\mu a\u/m - Ee/m$$
Setting this and $\u = 0 \implies \Frac{3Ee}{4\pi a^3} = g(\rho - \rho’) \implies$
$$\box{e = \Frac{4\pi a^3 g(\rho - \rho’)}{3E}}$$
<br>
...
__Sln__: The required model, $\u\Frac{d\u}{dx} = -\Frac{gR^2}{(R+x)^2}$, and its general solution, $\Frac{\u^2}2 = \Frac{gR^2}{R+x} + C$, are as given in the text (Eqs. 14 & 15), all that’s changed is the initial condition, $\u(x_0 = \xi R) = \u_0$, which gives $\Frac{\u_0^2}2 = \Frac{gR^2}{R+\xi R} + C \implies C = \Frac{\u_0^2}2 - \Frac{gR}{1+\xi} \implies$
$$\frac{\u^2}2 = \Frac{gR^2}{R+x} + \Frac{\u_0^2}2 - \Frac{gR}{1+\xi}$$
Escape velocity, $\u_e$, is the value of $\u_0$ that makes $\u = 0$ in the limit as $x \rarrow \infty$; this gives $0 = 0 + \Frac{\u_e^2}2 - \Frac{gR}{1+\xi} \implies$ 
$$\box{\u_e = \sqrt{\frac{2gR}{1+\xi}}}$$
Escape velocity at the surface of the earth is $\sqrt{2gR}$ (text Eq. 19) so we require $\Frac{\sqrt{2gR/(1+\xi)}}{\sqrt{2gR}} = (1+\xi)^{-1/2} = 0.85 \implies$ $\xi = (0.85)^{-2} - 1 \implies$ (using the Problem 15 approximation $R = 4000$ mi)
$$\box{x_0 = [(0.85)^{-2} - 1]R \doteq 1536\text{ mi}}$$
<br>
...
$$\left[1 + \left(\frac{dy}{dx}\right)^2\right]y = k^2~~~~~~~~~~\text{Eq. (i)}$$
$k^2 \gt 0$ constant.<br>

__a__) Solve (i) for $\Frac{dy}{dx}$ and explain why it is necessary to take the positive square root.<br>

__Sln__: $\left[1 + \left(\Frac{dy}{dx}\right)^2\right]y = k^2 \implies \left(\Frac{dy}{dx}\right)^2 = \Frac{k^2}y - 1 \implies \Frac{dy}{dx} = \pm\sqrt{\Frac{k^2}y - 1}$ and we must choose the positive sign
$$\box{\frac{dy}{dx} = \sqrt{\Frac{k^2}y - 1}}$$
because of the orientation of the axes: with $y$ increasing downward, $\Frac{dy}{dx} \gt 0$ in order for $y(x_0) = y_0 \lt 0.$
<br>
...
__Sln__: $y = k^2 \sin^2 t \implies \Frac{dy}{dt} = 2k^2\sin t\cos t$ and $\Frac{dy}{dx} = \Frac{dy}{dt}\Frac{dt}{dx} = (2k^2\sin t\cos t)\Frac{dt}{dx} = \sqrt{\Frac{k^2}{k^2\sin^2 t} - 1} = \sqrt{\csc^2 t - 1} = |\cot t| = \left|\Frac{\cos t}{\sin t}\right|$; now, since the right side must be positive and $2k^2\gt 0$ and $\Frac{dt}{dx} \gt 0$ (why?), $t$ must be such that $\sin t\cos t \gt 0 \implies \Frac{\cos t}{\sin t} \gt 0$ (why?) so $(2k^2\sin t\cancel{\cos t})\Frac{dt}{dx} = \Frac{\cancel{\cos t}}{\sin t} \implies
2k^2\sin^2 t dt = dx~~\blksq$
<br>
...
__Sln__: $x(\t=0) = 0 = y(\t=0)$ trivially, so the parameterization satisfies the IC; $x(\t) = k^2(\t - \sin\t)/2$ together with $\t = 2t \implies x(t) = k^2[2t - \sin(2t)]/2 \implies \Frac{dx}{dt} = \cancel{2}k^2[1-\cos(2t)]/\cancel{2} = k^2(1 - \cos^2 t + \sin^2 t) = 2k^2\sin^2 t$; $y(\t) = k^2(1 - \cos\t)/2$ together with $\t = 2t \implies y(t) = k^2[1 - \cos(2t)]/2 = k^2(1 - \cos^2 t + \sin^2 t)/2 = k^2\sin^2 t \implies \Frac{dy}{dt} = 2k^2\sin t\cos t$; and finally $\Frac{dy}{dx} = \Frac{dy}{dt}\Frac{dt}{dx} = \Frac{\cancel{2k^2\sin t}\cos t}{\cancel{2k^2}\sin^{\cancel{2}1} t} = \cot t = \sqrt{\csc^2 t - 1} = \sqrt{\Frac{k^2}{k^2\sin^2 t} - 1} = \sqrt{\Frac{k^2}{y} - 1}.~~\blksq$
<br><br>

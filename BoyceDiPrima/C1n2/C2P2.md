...
$\Frac{dN}{dt} = r(1-N/K)N-h$ adds a constant harvest rate to the logistic model.<br>
...
__c__) Graph $\Frac{dN}{dt}$ vs. $N$ and reason from it that if $N(0) = N_0 \gt N_1$ then $\lim_{t\rarrow\infty}N(t) = N_2$, but if $N_0 \lt N_1$ then $N$ is decreasing as $t$ increases $\implies N = 0$ for finite $t$ since $N=0$ is not an equilibrium point.<br> 

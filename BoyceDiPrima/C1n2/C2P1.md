...
__Sln__: &quot;Trivial&quot;: $\Frac{\int\mu(x)g(x)dx + c}{\mu(x)} = \Frac{\int\mu(x)g(x)dx}{\mu(x)} + \Frac c{\mu(x)} = cy_1(x) + y_2(x)$ with $\box{y_1 = \frac1{\mu(x)}}$ and $y_2 = \box{\frac{\int\mu(x)g(x)dx}{\mu(x)}}~~\blksq$.
<br>
...
__Sln__: I prefer the formalism of manipulating the equation to look like 
$M(x)dx + N(y)dy = 0 \implies \Int M(x)dx + \Int N(y)dy = C$;
this alternate formalism is implied in the text, and the reader should recognize that it’s &quot;six one way, half&#8209;a&#8209;dozen the other.&quot;&nbsp; Thus this DE is equivalent to $\Frac{x^2}{1+x^3}dx - ydy = 0 \implies$ $\displaystyle \int \frac{x^2}{1+x^3}dx - \int ydy$, which, letting $u = 1+x^3$, $= \Frac13\int \frac{du}u - \frac12y^2 = \frac13\log|1+x^3| - \frac12 y^2 = C’ \implies$
$$\box{2\log|1+x^3| - 3y^2 = C}$$
where both the original DE and the solution dictate that $x \ne -1$ and the DE dictates that $y \ne 0$.<br><br>
Check: Rewriting $|1+x^3| = \sqrt{\left(1+x^3\right)^2} \implies 2\log|1+x^3| = \cancel{2(\Frac12)}\log\left[(1+x^3)^2\right] \implies d\left\{\log\left[(1+x^3)^2\right]\right\} = \Frac{2\cancel{(1+x^3)}(3x^2)}{(1+x^3)^{\cancel{2}1}}dx$, while $d(-3y^2) = -6ydy$ so $d(2\log|1+x^3| - 3y^2 = C) \implies \Frac{6x^2}{1+x^3}dx - 6ydy = 0 \implies \Frac{dy}{dx} = \Frac{x^2}{y(1+x^3)}~~\chec$.
<br><br>
...
__Sln__: We begin by noting that $\t$ can’t equal $0$, and, excluding that, if $r = 0, dr/d\t = 0$, so when we get the solution, we’ll have to check it for consistency with that.&nbsp; For $r, \t \ne 0, dr/d\t = r^2/\t \implies r^{-2}dr - d\t/\t = 0 \implies -\Frac1r - \log|\t| = C$; the initial condition gives $-\Frac12 - \log(1) = -\Frac12 = C \implies$ $\Frac1r + \Frac12\log(\t^2) = \Frac12 \implies \Frac1r = \Frac{1-\log(\t^2)}2 \implies r = \Frac2{1-\log(\t^2)}$, which, in addition to the already stipulated $\t \ne 0$, also requires that $\log(\t^2) \ne 1 \implies \t \ne \pm\sqrt{e}$; $r$ is well&#8209;defined for all other values of $\t$, but, we note, is never $0$.&nbsp; Boxing it all up, we have that the solution is:
$$\box{r(\t) = \frac2{1-\log(\t^2)}, \t \ne 0, \pm\sqrt{e}}$$
...
__Sln__: We begin by noting that $y$ can’t equal $0$.&nbsp; Otherwise, the DE is equivalent to $(x^3 + x)dx - 4y^3dy = 0 \implies x^4/4 + x^2/2 - y^4 = C$; the initial condition requires $0^4/4 + 0^2/2 - (-1/\sqrt2)^4 = -1/4 = C \implies y^4 = x^4/4 + x^2/2 + 1/4$ which implies, formally, $y = \pm\sqrt[4]{x^4/4 + x^2/2 + 1/4}$; however, the initial condition requires that we choose the negative sign; since every term under the radical is $\ge 0$, $y$ is well&#8209;defined for all real $x$; moreover, since $x^4/4 + x^2/2 + 1/4 \ge 1/4$ (why? proof?), $y \ne 0$ for all real $x$; therefore, the solution is:
...
__Sln__: Begin by noting that $y$ can’t equal $\pm2/\sqrt3$; excluding those values of $y$, the DE is equivalent to $3x^2dx - (3y^2 - 4)dy = 0 \implies x^3 - y^3 + 4y = C$; the IC gives $1^3 - 0^3 + 4(0) = 1 = C$, which implies the solution, expressed implicitly (we are not asked to express the solution explicitly), is $\box{x^3 - y^3 + 4y = 1}$.&nbsp; Now, ___if___ the solution were well&#8209;defined at $y = \pm2/\sqrt3$, the corresponding values of $x$ would be determined from $x^3 = (\pm2/\sqrt3)^3 - 4(\pm2/\sqrt3) + 1 = \pm8/(3\sqrt3) \mp 24/(3\sqrt3) + 1 = 1 \mp16/(3\sqrt3)$ so the points excluded from the solution are $\box{\left(\sqrt[3]{1 - 16/(3\sqrt3)}, 2/\sqrt3\right)}$ and $\box{\left(\sqrt[3]{1 + 16/(3\sqrt3)}, -2/\sqrt3\right)}$.<br><br>
Here’s a graph of the solution with the undefined points indicated $\color{red}{\text{in red}}$ (the reader should pretend these are open circles with breaks in the solution curve at these points); note that the solution would have a vertical slope, i.e., an undefined derivative, if it were well&#8209;defined at these points; note also that, if one wanted to go to the trouble of expressing this solution &quot;explicitly&quot;, it would make more sense, as well as be more straightforward, to express it as $x(y)$ instead of as $y(x)$ (obviously, $y$ is not a function of $x$ in the strict sense of that term).
<br><br>
<img src="./Graphs/BoyceDiPrimaC2S3P18.png">
<br>
...
$$\box{|y-2x||y+2x|^3 = C}$$
<br>
Check: the tedious&#8209;but&#8209;straighforward check is left to the reader (hints: $|x| = \sqrt{x^2}$, and remember that after differentiating the right hand side of the solution, it’s $0$, so a lot of &quot;complications&quot; can be eliminated by multiplying and dividing through by factors that arise in differentiating the left hand side&mdash;have fun!)
<br><br>
&quot;__Extra Credit__&quot;: Explain the significance of the four lines: $y = x$; $y = 4x$; $y = 2x$; and $y = -2x$.
<br><br>
...
__Sln__: $y’ = f(x,y) = (1 - x^2 - y^2)^{1/2}$ is continuous for $1 - x^2 - y^2 \ge 0 \implies x^2 + y^2 \le 1$; $\partial_y f = \Frac12(1 - x^2 - y^2)^{-1/2}(-2y) = -\Frac y{\sqrt{1 - x^2 - y^2}}$ is continuous for $1 - x^2 - y^2 \gt 0 \implies x^2 + y^2 \lt 1$; the latter of these is the intersection of the two regions, thus the required region is 
...
__Sln__: Verification that the IC is satisfied for both functions is trivial and left to the reader. For the DEs: $y_1’ = -1$ while $\Frac{-x + (x^2 + 4(1-x))^{1/2}}{2} = \Frac{-x + (x^2 - 4x + 4)^{1/2}}2 = \Frac{-x + |x-2|}2$ and for this to $ = -1$ identicallly, $x - 2 \ge 0 \implies \box{x \ge 2}$ as the domain of validity for $y_1$. For $y_2, y_2’ = -x/2$ while $\Frac{-x + (x^2 + 4(-x^2/4))^{1/2}}{2} = \Frac{-x + (x^2 - x^2)^{1/2}}2 \equiv -x/2~\box{\text{for all }x}$.
<br>

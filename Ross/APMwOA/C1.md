...
<title>Solutions of Selected Problems from Ross, S. "Applied. Prob. Models w/ Optim. Applications, Chapter 1"</title>
...
<table>
  ...
  <tr>
    <td><a href="#P9">Problem 9</a></td>
    <td><a href="#P10">Problem 10</a></td>
    <td><a href="#P11">Problem 11</a></td>
    <td><a href="#P13">Problem 13</a></td>
  </tr>
</table>
</div>

<a name="P3" class="goback" onclick="winhisback()">__3__)</a>
Given $X, Y$ independent Poisson RV's, find the distribution of $X + Y$.
...
<a name="P4" class="goback" onclick="winhisback()">__4__)</a>
Given $X \sim \text{POIS}(\lambda_1), Y \sim \text{POIS}(\lambda_2)$, show that $(X \mid X + Y) \sim \text{BIN}(n,p)$ for some whole number $n$, and some real number $p : 0 \lt p \lt 1$. 
...
Find the distribution of $X$, given that $X \sim \text{POIS}$ with $EX=\lambda \sim \text{EXP}$ with $E\lambda = 1/\mu$.
...
Let $N$ denote the number of customers arriving at a store in a given day. Suppose that the amounts spent by the customers are independent and have a [common] distribution $F$. Find the mean and variance of the total amount of money spent in the store [per day].
...
Show that any Process with independent increments is Markov.
...
Let $Y_{1,n}, Y_{2,n},…, Y_{n,n}$ be $n$ independent random variables with identical [continuous] uniform distibution on $(0,t)$ [i.e., relative to the notation used in the text's Table 2, $a = 0$ and $b = t$]. Let $Z_n = \min(Y_{1,n},…, Y_{n,n})$.
...
Show for the Wiener process that Var$~X(t) = \sigma^2 t$ for some $\sigma ^2 > 0.$

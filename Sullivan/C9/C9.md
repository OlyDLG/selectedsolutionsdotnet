...
__Pf__: We may assume that none of the $a,b,c = 0$ (why?), so multiplying through by $2abc$ we see that the claim is equivalent to: $a^2 + b^2 + c^2 = 2bc\cos A + 2ac\cos B + 2ab\cos C$; but by the LoC, this right-hand-side is equal to: $(b^2+c^2-a^2) + (a^2+c^2-b^2) + (a^2+b^2-c^2) =\\ b^2+c^2-\cancel{a^2}+\cancel{a^2}+\cancel{c^2}-\cancel{b^2}+a^2+\cancel{b^2}-\cancel{c^2}.~~~\blacksquare$
<br><br>
...
So, using the formula, the answer for the given data is $\displaystyle \frac{25}{2} (40\frac{\pi}{180}-\sin 40°) \doteq \boxed{0.692\text{ sq. in.}}$
<br><br>
...
Referring to the figure, calculate: __a__) $\text{Area }\triangle OBC$; and __b__) $\text{Area } \triangle OBD$; both in terms of $\theta$, and then __c__) use these results and the fact that $\text{Area }\triangle OBC \lt \text{Area }\overset{\frown}{OBC} \lt \text{Area } \triangle OBD$, where $\overset{\frown}{OBC}$ is the circular sector $OBC$ and its area is given to be $\frac{\theta}2, \theta$ (must be) in radians, to show that: $$1 \lt \frac{\theta}{\sin \theta} \lt \frac1{\cos \theta}.$$
...
__Sln__: __a__) This is still SHM...but not around $d=0$; rather, it is &quot;around&quot; $d=4$, with a maximum of $d=4+3=7$ meters and a minimum of $d=4-3=1$ meter; __b__) The maximum displacement from equilibrium, however, is still just the amplitude, $\boxed{3\text{ meters}}$; __c__) $\omega=\pi$ radians per second so the time required for one complete oscillation, i.e., the period, is $\frac{2\pi}{\pi} = \boxed{2\text{ seconds per cycle}}$; and __d__) the frequency is just the reciprocal of the period, so $\boxed{\frac12\text{ cycles per second = 0.5 Hz.}}$
<br><br>
